...
const MovieDetails: React.FC<MovieDetailsProps> = ({ movie, onClose, botUsername, channelLink }) => {
  ...
  return (
    <>
      ...
      >
        {/* Detail Banner - 2:3 portrait ratio */}
        <div className="absolute top-0 left-0 w-full z-0" style={{ aspectRatio: '2/3', maxHeight: '58vh' }}>
          <img
            src={movie.thumbnail}
            alt={movie.title}
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#000] via-[#000]/75 to-transparent" />
          <div className="absolute inset-0 bg-gradient-to-r from-[#000]/25 to-transparent" />
        </div>
        ...
        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto no-scrollbar relative z-10" style={{ paddingTop: 'min(55vw, 50vh)' }}>
                        ...
                        {currentEpisodes.length > 0 ? currentEpisodes.map((ep, index) => {
                          ...
                          return (
                            <motion.div
                              ...
                              key={ep.id}
                              className={`group rounded-2xl overflow-hidden bg-gradient-to-br from-[#0f0f0f] to-[#141414] border transition-all ${
                                isLocked ? 'border-white/5 opacity-70' : 'border-white/8 hover:border-gold/30 hover:shadow-[0_0_20px_rgba(255,215,0,0.1)]'
                              }`}
                            >
                              {/* Episode Thumbnail - 16:9 Bigger HD */}
                              <div
                                onClick={() => !isLocked && handleWatch(ep.telegramCode)}
                                className={`relative w-full overflow-hidden ${isLocked ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                                style={{ aspectRatio: '16/9' }}
                              >
                                <img
                                  src={ep.thumbnail || movie.thumbnail}
                                  className="absolute inset-0 w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
                                  style={{ filter: ep.thumbnail ? 'none' : 'brightness(0.35) blur(1px)' }}
                                  alt={ep.title}
                                />
                                ...
                                {isLocked ? (
                                  <div className="absolute inset-0 bg-black/70 backdrop-blur-sm flex flex-col items-center justify-center gap-2">
                                    ...
                                    <span className="text-xs text-yellow-400 font-bold">LOCKED</span>
                                  </div>
                                ) : (
                                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 flex items-center justify-center transition-all duration-300">
                                    <div className="w-14 h-14 rounded-full bg-white/0 group-hover:bg-gold/90 backdrop-blur-sm flex items-center justify-center transition-all duration-300 shadow-[0_0_20px_rgba(255,215,0,0.3)]">
                                      <Play size={20} fill="black" className="text-black opacity-0 group-hover:opacity-100 transition-opacity ml-1" />
                                    </div>
                                  </div>
                                )}
                                
                                {/* Episode number badge */}
                                <div className="absolute top-3 left-3 bg-black/80 backdrop-blur-md text-xs font-bold text-white px-3 py-1.5 rounded-lg border border-white/20">
                                  Episode {ep.number}
                                </div>
                                
                                {/* Duration badge - top right */}
                                {ep.duration && !isLocked && (
                                  <div className="absolute top-3 right-3 flex items-center gap-1.5 bg-black/80 backdrop-blur-md px-2.5 py-1.5 rounded-lg border border-white/20">
                                    <Clock size={12} className="text-blue-400" />
                                    <span className="text-xs font-bold text-white">{ep.duration}</span>
                                  </div>
                                )}
                              </div>

                              {/* Episode Info & Buttons */}
                              <div className="p-4">
                                {/* Title */}
                                <div className="mb-3">
                                  <h4 className={`text-sm font-bold leading-snug mb-1 ${isLocked ? 'text-gray-400' : 'text-white group-hover:text-gold transition-colors'}`}>
                                    {ep.title}
                                  </h4>
                                    ...
                                    <div className="flex items-center gap-2 mt-2">
                                      {ep.releaseDate && (
                                        <div className="flex items-center gap-1.5 bg-yellow-500/10 border border-yellow-500/30 px-2 py-1 rounded-md">
                                          ...
                                          <span className="text-[10px] text-yellow-400 font-semibold">{ep.releaseDate}</span>
                                        </div>
                                      )}
                                      {!ep.releaseDate && (
                                        <span className="text-[10px] bg-yellow-500/20 text-yellow-400 px-2.5 py-1 rounded-full border border-yellow-500/30 font-bold">
                                          COMING SOON
                                        </span>
                                      )}
                                    </div>
                                ...
                                {/* Metadata tags - professional */}
                                {!isLocked && (
                                  <div className="flex items-center gap-2 flex-wrap mb-3">
                                    {ep.quality && (
                                      <span className="flex items-center gap-1 text-[10px] font-semibold text-blue-400 bg-blue-500/10 px-2 py-1 rounded-md border border-blue-500/20">
                                        <Tv size={9} />{ep.quality}
                                      </span>
                                    )}
                                    {ep.fileSize && (
                                      <span className="flex items-center gap-1 text-[10px] font-semibold text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded-md border border-emerald-500/20">
                                        <Database size={9} />{ep.fileSize}
                                      </span>
                                    )}
                                    {ep.audioLanguage && (
                                      <span className="flex items-center gap-1 text-[10px] font-semibold text-purple-400 bg-purple-500/10 px-2 py-1 rounded-md border border-purple-500/20">
                                        <Volume2 size={9} />{ep.audioLanguage}
                                      </span>
                                    )}
                                  </div>
                                )}

                                {/* Action Buttons - Bigger and Better */}
                                <div className="flex items-center gap-2">
                                  ...
                                  <button
                                    ...
                                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-xs font-bold transition-all ${
                                      isLocked
                                        ? 'bg-gray-800 border border-gray-700 cursor-not-allowed opacity-40 text-gray-600'
                                        : 'bg-[#1a1a1a] hover:bg-[#252525] border border-white/10 text-gray-300 hover:text-white hover:border-white/20 active:scale-95'
                                    }`}
                                  >
                                  ...
                                  <button
                                    ...
                                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-xs font-bold transition-all ${
                                      isLocked
                                        ? 'bg-gray-800 border border-gray-700 cursor-not-allowed opacity-40 text-gray-600'
                                        : 'bg-gold text-black hover:bg-[#ffe033] border border-gold active:scale-95 shadow-[0_0_15px_rgba(255,215,0,0.2)]'
                                    }`}
                                  >
                          ...
                          <div className="text-center py-16 text-gray-500 text-sm">
                            ...
                            <p className="font-semibold">No episodes available for Season {selectedSeason}</p>
                          </div>
                        )}
                      </div>
                    )}
